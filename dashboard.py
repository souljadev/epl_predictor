...
for _, row in df.iterrows():
    ...
    overview_rows.append({
        ...
        "Score Probability": f"{row['most_likely_score_prob']*100:.1f}%"
    })
...
match_list = df["HomeTeam"] + " vs " + df["AwayTeam"]
selection = st.selectbox("Choose a match:", match_list)

home_selected = selection.split(" vs ")[0]
away_selected = selection.split(" vs ")[1]

row = df[(df["HomeTeam"] == home_selected) & (df["AwayTeam"] == away_selected)].iloc[0]


# ===============================================
# 1X2 PROBABILITIES
# ===============================================
st.subheader(f"📌 {home_selected} vs {away_selected}")
...
with colB:
    st.metric("Probability", f"{row['most_likely_score_prob']*100:.1f}%")
with colC:
...
matrix = exact[(exact["HomeTeam"] == home_selected) & (exact["AwayTeam"] == away_selected)]
...
st.write("Probability for each exact score:")

st.table((pivot * 100).round(2))


# ===============================================
# AI MATCH INTERPRETATION
# ===============================================
st.markdown("---")
...
def interpret(row):
    ...
    return (
        ...
        f"**Most likely score:** {row['most_likely_score']} ({row['most_likely_score_prob']*100:.1f}%)\n"
